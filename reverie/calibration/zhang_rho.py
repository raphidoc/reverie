...
def load():
    ...
    db_path = "/home/raphael/PycharmProjects/reverie/reverie/data/lut/Zhang_rho_db.mat"
...
def get_sky_sun_rho(
    ...
    # Change units
    view_ang2 = view_ang + np.array([0, 180])
    view_pol = np.deg2rad(view_ang)
    view_vec = my_sph2cart(view_pol[1], view_pol[0])
    view_pol2 = np.deg2rad(view_ang2)
    view_loc2 = find_quads(view_pol2[0], view_pol2[1])

    # Set output
    rho = {}

    # Probability and reflection angle of reflecting skylight into the sensor
    logger.debug(f"Get prob & ang ({wind_speed}, {view_ang})")
    prob, angr_sky = get_prob(wind_speed, tuple(view_vec))
    tprob = np.sum(prob, 0)
    ref = sw_fresnel(wavelength, angr_sky, water_temperature, water_salinity)

    # Sky radiance
    # TODO Check dtype of skyrad0 to lower memory footprint
    # TODO Look into numexpr, numba, dask library
    logger.debug(f"Interpolate skyrad ({sun_zen}', '{aot_550}', '{wavelength[0:5]}...)")
    db_idx = np.arange(skyrad0.shape[2])
    # xi = np.array(np.meshgrid(sun_zen, aot_550, db_idx, wavelength, copy=False)).T
    ...
    skyrad = interpn_chunked((db['zen_sun'], db['od'], db_idx, db['wv']), skyrad0,
                             (sun_zen, aot_550, db_idx, wavelength), chunked_axis=2).squeeze()
    ...
    xi = np.array(np.meshgrid(sun_zen, aot_550, wavelength, copy=False)).T.reshape(-1, 3)
    sunrad = interpn((db['zen_sun'], db['od'], db['wv']), sunrad0, xi.reshape(-1, 3)).reshape(xi.shape[:-1]).T.squeeze()

    sun_vec = gen_vec_polar(np.deg2rad(sun_zen))
    prob_sun, angr_sun = prob_reflection(-sun_vec, view_vec, wind_speed)
    ref_sun = sw_fresnel(wavelength, angr_sun, water_temperature, water_salinity)
    ...
    xi = np.array(np.meshgrid(wind_speed, aot_550, sun_zen,
                              wavelength, 180 - view_ang[0], 180 - view_ang[1], copy=False)).T
    rad_inc_sca = interpn(x, rad_boa_sca, xi.reshape(-1, 6)).reshape(xi.shape[:-1]).T.squeeze()
    xi = np.array(np.meshgrid(wind_speed, aot_550, sun_zen,
                              wavelength, view_ang[0], 180 - view_ang[1]), copy=True).T
    ...
    xi = np.array(np.meshgrid(wind_speed, aot_550, sun_zen,
                              wavelength, view_ang[0], 180 - view_ang[1], copy=False)).T
