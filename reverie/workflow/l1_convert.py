...
def l1_convert(bundle_path: str = None, sensor: str = None):
    ...
    l1 = wise.read_pix.Pix(image_dir='/D/Data/TEST/TEST_WISE/MC-50/', image_name='190818_MC-50A-WI-2x1x1_v02')
    l1.create_dataset_nc(out_file=os.path.join(l1.image_dir, l1.image_name) + '-L1C.nc')

    # Create radiometric variable
    l1.create_var_nc(var='Lt', dimensions=('W', 'Y', 'X',))
    ...
    for var in tqdm(geom, desc='Writing geometry'):
        l1.create_var_nc(var=var, dimensions=('Y', 'X',))
