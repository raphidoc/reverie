...
from reverie.utils.helper import rm_illegal_sym_envi_hdr
...
class FlightLine():
    ...
    @classmethod
    def FromWISEFile(cls, nav_sum_log, glu_hdr):
        """
        ...
        def read():
            try:
                header = envi.read_envi_header(glu_hdr)
            except FileNotAnEnviHeader as e:
                #_logger.warning(e)
                new_lines = rm_illegal_sym_envi_hdr(glu_hdr)
                new_hdr = glu_hdr.replace('.hdr', '.copt.hdr')
